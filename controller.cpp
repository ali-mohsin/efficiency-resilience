...
Controller::Controller(int kay,int tor,int aggr,int core,int back,int share, int runFor,int makeFlows)
{
	...
	for(int i=0;i<all_links.size();i++)
	{
		...
		int primary=all_links[i]->total_cap - all_links[i]->available_cap_up - all_links[i]->backup_up;
		int secondary=all_links[i]->backup_up;
		cout<<"ID: " << all_links[i]->link_id<<" Primary: "<< primary << " Secondary: "<< secondary<<endl;
	...
	for(int i=0; i<len; i++)
	{
		if( all_switches[i]->getStatus() < 0 )
		{
			...
			for(int j=0;j<flows_primary.size();j++)
			{
				...
				flows_primary[j]->antiCommitPath(flows_primary[j]->primaryPath,0);
				if(backUp)
				{
			...
			for(int j=0;j<flows_back.size();j++)
			{
				if(backUp)
				{
					...
					flows_back[j]->antiCommitPath(flows_back[j]->backUpPath,0);
...
void Controller::revert_to_primary()
{
	...
	if(backUp)
	{
		for (int i=0;i<flows_on_back.size();i++)
		{
			...
			{
				f->antiCommitPath(f->backUpPath,0); //value will be one when flow is ended
